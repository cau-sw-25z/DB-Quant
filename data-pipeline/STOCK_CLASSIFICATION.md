...
종목별 가격 패턴과 변동성 특성을 분석하여 4가지 매매전략 유형 중 하나로 분류한다.
분류 결과는 `stock_classification` 테이블에 저장되며, 3주차 종목별 전략 매핑(DB-09)의 직접 입력값으로 사용된다.

- **분류 스크립트**: `data-pipeline/quant/classify_stock.py`
...
```
검사 순서: VOLATILITY_BREAKOUT → MOMENTUM → TREND_FOLLOWING → MEAN_REVERSION → UNCLASSIFIED
```

> 우선순위가 높은 유형일수록 특징이 뚜렷하고 강한 신호를 가진다.
> UNCLASSIFIED 종목은 현재 4가지 전략에 해당하지 않으며, 추후 새로운 전략 추가 시 흡수된다.
...
FE 종목 상세 화면에 전략 유형 표시 시 위 5가지 값 중 하나가 내려간다.