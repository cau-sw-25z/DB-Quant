...
MIN_DAILY_VALUE = 10_000_000_000  # 50억원
...
class DailyScreener:
    ...
    def _load_all_at_once(self):
        universe_query = text("""
            ...
            WHERE sc.strategy_type NOT IN ('UNCLASSIFIED', 'VOLATILITY_BREAKOUT', 'MOMENTUM')
        """)
        ...
        try:
            ...
            print("  → portfolios 테이블 없음. 매수 스캔만 진행.")
    ...
    def _check_data_integrity(self, df: pd.DataFrame) -> bool:
        """액면분할 등 비정상 틱 감지. 전일 대비 31% 초과 등락이면 False."""
    ...
    def _check_liquidity(self, df: pd.DataFrame) -> bool:
        """
        최근 20일 평균 거래대금이 MIN_DAILY_VALUE 이상인지 확인.
        거래대금 = 종가 × 거래량
        유동성 부족 종목은 시가 매수 시 슬리피지 발생 가능.
        """
        recent = df.tail(20).copy()
        recent['daily_value'] = recent['close_price'] * recent['volume']
        avg_value = recent['daily_value'].mean()
        return avg_value >= MIN_DAILY_VALUE
    ...
    def analyze_single_ticker(self, ticker: str) -> dict | None:
        ...
        if not self._check_liquidity(df):
            return None
    ...
    def run_full_scan(self, max_workers=10) -> pd.DataFrame:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(as_completed(future_map), total=len(self.tickers),
                               desc="스캔 진행률", ncols=80):
